...
目前只能通过观察graph内部的计数器，来判断整个数据迁移工作是否完成；观察方法如下：对所有新扩容的graph实例，访问其统计接口http://127.0.0.1:6071/counter/migrate（这个地址是新扩容graph的http接口，所以需要自己变通） 观察到所有的计数器都不再变化，那么就意味着迁移工作完成啦。
